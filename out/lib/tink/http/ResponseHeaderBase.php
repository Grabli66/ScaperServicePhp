<?php
/**
 * Generated by Haxe 4.0.0-rc.3+e3df7a4
 */

namespace tink\http;

use \php\_Boot\HxAnon;
use \tink\io\StreamParserObject;
use \php\Boot;
use \tink\core\TypedError;
use \tink\core\Outcome;
use \php\_Boot\HxString;
use \httpstatus\_HttpStatusMessage\HttpStatusMessage_Impl_;

class ResponseHeaderBase extends Header {
	/**
	 * @var string
	 */
	public $protocol;
	/**
	 * @var string
	 */
	public $reason;
	/**
	 * @var int
	 */
	public $statusCode;

	/**
	 * @return StreamParserObject
	 */
	static public function parser () {
		#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:51: lines 51-58
		return new HeaderParser(function ($line, $headers) {
			#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:52: characters 21-36
			$_g = HxString::split($line, " ");
			#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:53: lines 53-56
			if ($_g->length >= 3) {
				#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:54: characters 19-94
				$statusCode = \Std::parseInt(($_g->arr[1] ?? null));
				$reason = $_g->slice(2)->join(" ");
				#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:54: characters 11-95
				return Outcome::Success(new ResponseHeaderBase($statusCode, $reason, $headers, ($_g->arr[0] ?? null)));
			} else {
				#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:56: characters 11-82
				return Outcome::Failure(new TypedError(422, "Invalid HTTP response header", new HxAnon([
					"fileName" => "tink/http/Response.hx",
					"lineNumber" => 56,
					"className" => "tink.http.ResponseHeaderBase",
					"methodName" => "parser",
				])));
			}
		});
	}

	/**
	 * @param int $statusCode
	 * @param string $reason
	 * @param \Array_hx $fields
	 * @param string $protocol
	 * 
	 * @return void
	 */
	public function __construct ($statusCode, $reason = null, $fields = null, $protocol = "HTTP/1.1") {
		#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:37: lines 37-42
		if ($protocol === null) {
			$protocol = "HTTP/1.1";
		}
		#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:38: characters 5-33
		$this->statusCode = $statusCode;
		#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:39: characters 5-55
		$this->reason = ($reason === null ? HttpStatusMessage_Impl_::fromCode($statusCode) : $reason);
		#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:40: characters 5-29
		$this->protocol = $protocol;
		#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:41: characters 5-18
		parent::__construct($fields);
	}

	/**
	 * @param \Array_hx $fields
	 * 
	 * @return ResponseHeaderBase
	 */
	public function concat ($fields) {
		#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:45: characters 12-88
		$statusCode = $this->statusCode;
		$reason = $this->reason;
		$fields1 = $this->fields->concat($fields);
		return new ResponseHeaderBase($statusCode, $reason, $fields1, $this->protocol);
	}

	/**
	 * @return string
	 */
	public function toString () {
		#/home/grabli66/haxelib/tink_http/git/src/tink/http/Response.hx:48: characters 5-82
		return "" . ($this->protocol??'null') . " " . ($this->statusCode??'null') . " " . ($this->reason??'null') . "\x0D\x0A" . (parent::toString()??'null');
	}

	public function __toString() {
		return $this->toString();
	}
}

Boot::registerClass(ResponseHeaderBase::class, 'tink.http.ResponseHeaderBase');
