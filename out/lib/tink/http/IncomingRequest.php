<?php
/**
 * Generated by Haxe 4.0.0-rc.3+e3df7a4
 */

namespace tink\http;

use \php\_Boot\HxAnon;
use \tink\core\_Future\SyncFuture;
use \tink\io\RealSourceTools;
use \php\Boot;
use \tink\io\_Source\Source_Impl_;
use \tink\core\TypedError;
use \tink\streams\StreamObject;
use \tink\core\Outcome;
use \tink\core\_Lazy\LazyConst;
use \php\_Boot\HxString;
use \tink\core\_Promise\Promise_Impl_;
use \tink\core\_Future\FutureObject;

class IncomingRequest extends Message {
	/**
	 * @var string
	 */
	public $clientIp;

	/**
	 * @param string $clientIp
	 * @param StreamObject $source
	 * 
	 * @return FutureObject
	 */
	static public function parse ($clientIp, $source) {
		#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:130: lines 130-145
		return Promise_Impl_::next(RealSourceTools::parse($source, IncomingRequestHeader::parser()), function ($parts)  use (&$clientIp) {
			#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:134: characters 11-18
			$parts1 = $parts->a;
			#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:135: characters 24-50
			$_g = $parts->a->getContentLength();
			#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:135: lines 135-144
			$d = null;
			#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:135: characters 24-50
			$__hx__switch = ($_g->index);
			if ($__hx__switch === 0) {
				#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:135: lines 135-144
				$d = Source_Impl_::limit($parts->b, $_g->params[0]);
			} else if ($__hx__switch === 1) {
				#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:139: characters 39-72
				$_g1 = $parts->a->byName("transfer-encoding");
				#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:139: characters 23-37
				$__hx__switch = ($parts->a->method);
				if ($__hx__switch === "GET" || $__hx__switch === "OPTIONS") {
					#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:135: lines 135-144
					$d = Source_Impl_::$EMPTY;
				} else {
					#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:139: characters 39-72
					if ($_g1->index === 0) {
						#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:141: characters 34-77
						$_this = HxString::split($_g1->params[0], ",");
						$f = Boot::getStaticClosure(\StringTools::class, 'trim');
						$result = [];
						$i = 0;
						while ($i < $_this->length) {
							$result[] = $f($_this->arr[$i]);
							++$i;
						}
						#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:141: lines 141-142
						if (\Array_hx::wrap($result)->indexOf("chunked") !== -1) {
							#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:141: characters 133-156
							$source1 = $parts->b;
							#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:135: lines 135-144
							$d = Chunked::decoder()->transform($source1);
						} else {
							#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:142: characters 25-79
							return new SyncFuture(new LazyConst(Outcome::Failure(new TypedError(411, "Content-Length header missing", new HxAnon([
								"fileName" => "tink/http/Request.hx",
								"lineNumber" => 142,
								"className" => "tink.http.IncomingRequest",
								"methodName" => "parse",
							])))));
						}
					} else {
						return new SyncFuture(new LazyConst(Outcome::Failure(new TypedError(411, "Content-Length header missing", new HxAnon([
							"fileName" => "tink/http/Request.hx",
							"lineNumber" => 142,
							"className" => "tink.http.IncomingRequest",
							"methodName" => "parse",
						])))));
					}
				}
			}
			#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:132: lines 132-145
			return new SyncFuture(new LazyConst(Outcome::Success(new IncomingRequest($clientIp, $parts1, IncomingRequestBody::Plain($d)))));
		});
	}

	/**
	 * @param string $clientIp
	 * @param IncomingRequestHeader $header
	 * @param IncomingRequestBody $body
	 * 
	 * @return void
	 */
	public function __construct ($clientIp, $header, $body) {
		#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:125: characters 5-29
		$this->clientIp = $clientIp;
		#/home/grabli66/haxelib/tink_http/git/src/tink/http/Request.hx:126: characters 5-24
		parent::__construct($header, $body);
	}
}

Boot::registerClass(IncomingRequest::class, 'tink.http.IncomingRequest');
