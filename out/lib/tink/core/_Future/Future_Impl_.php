<?php
/**
 * Generated by Haxe 4.0.0-rc.3+e3df7a4
 */

namespace tink\core\_Future;

use \haxe\Timer;
use \tink\core\_Lazy\LazyObject;
use \php\Boot;
use \haxe\ds\Either;
use \tink\core\Noise;
use \tink\core\_Callback\Callback_Impl_;
use \tink\core\Outcome;
use \tink\core\FutureTrigger;
use \tink\core\_Lazy\LazyConst;
use \tink\core\OutcomeTools;
use \tink\core\MPair;
use \tink\core\_Outcome\OutcomeMapper_Impl_;

final class Future_Impl_ {
	/**
	 * @var FutureObject
	 */
	static public $NEVER;
	/**
	 * @var FutureObject
	 */
	static public $NOISE;
	/**
	 * @var FutureObject
	 */
	static public $NULL;

	/**
	 * @param FutureObject $f
	 * @param \Closure $map
	 * 
	 * @return FutureObject
	 */
	static public function _flatMap ($f, $map) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:177: characters 12-26
		return $f->flatMap($map)->gather();
	}

	/**
	 * @param FutureObject $f
	 * @param \Closure $map
	 * 
	 * @return FutureObject
	 */
	static public function _map ($f, $map) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:180: characters 12-22
		return $f->map($map)->gather();
	}

	/**
	 * @param \Closure $f
	 * 
	 * @return FutureObject
	 */
	static public function _new ($f) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:12: character 3
		return new SimpleFuture($f);
	}

	/**
	 * @param FutureObject $f
	 * @param \Closure $map
	 * 
	 * @return FutureObject
	 */
	static public function _tryFailingFlatMap ($f, $map) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:159: lines 159-162
		return $f->flatMap(function ($o)  use (&$map) {
			$__hx__switch = ($o->index);
			if ($__hx__switch === 0) {
				#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:160: characters 24-30
				return $map($o->params[0]);
			} else if ($__hx__switch === 1) {
				#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:161: characters 24-47
				return new SyncFuture(new LazyConst(Outcome::Failure($o->params[0])));
			}
		})->gather();
	}

	/**
	 * @param FutureObject $f
	 * @param \Closure $map
	 * 
	 * @return FutureObject
	 */
	static public function _tryFailingMap ($f, $map) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:171: characters 12-53
		return $f->map(function ($o)  use (&$map) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:171: characters 31-52
			return OutcomeTools::flatMap($o, OutcomeMapper_Impl_::withSameError($map));
		})->gather();
	}

	/**
	 * @param FutureObject $f
	 * @param \Closure $map
	 * 
	 * @return FutureObject
	 */
	static public function _tryFlatMap ($f, $map) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:165: lines 165-168
		return $f->flatMap(function ($o)  use (&$map) {
			$__hx__switch = ($o->index);
			if ($__hx__switch === 0) {
				#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:166: characters 24-43
				return $map($o->params[0])->map(Boot::getStaticClosure(Outcome::class, 'Success'))->gather();
			} else if ($__hx__switch === 1) {
				#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:167: characters 24-47
				return new SyncFuture(new LazyConst(Outcome::Failure($o->params[0])));
			}
		})->gather();
	}

	/**
	 * @param FutureObject $f
	 * @param \Closure $map
	 * 
	 * @return FutureObject
	 */
	static public function _tryMap ($f, $map) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:174: characters 12-49
		return $f->map(function ($o)  use (&$map) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:174: characters 31-48
			return OutcomeTools::map($o, $map);
		})->gather();
	}

	/**
	 *  Uses `Pair` to merge two futures
	 * 
	 * @param FutureObject $a
	 * @param FutureObject $b
	 * 
	 * @return FutureObject
	 */
	static public function and ($a, $b) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:156: characters 5-61
		return Future_Impl_::merge($a, $b, function ($a1, $b1) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:156: characters 46-60
			return new MPair($a1, $b1);
		});
	}

	/**
	 *  Casts a Surprise into a Promise
	 * 
	 * @param FutureObject $s
	 * 
	 * @return FutureObject
	 */
	static public function asPromise ($s) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:88: characters 5-13
		return $s;
	}

	/**
	 *  Creates an async future
	 *  Example: `var i = Future.async(function(cb) cb(1)); // Future<Int>`
	 * 
	 * @param \Closure $f
	 * @param bool $lazy
	 * 
	 * @return FutureObject
	 */
	static public function async ($f, $lazy = false) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:131: lines 131-138
		if ($lazy === null) {
			$lazy = false;
		}
		if ($lazy) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:132: characters 7-32
			return new LazyTrigger($f);
		} else {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:134: characters 7-26
			$op = new FutureTrigger();
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:136: characters 7-33
			Callback_Impl_::invoke($f, Boot::getInstanceClosure($op, 'trigger'));
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:137: characters 7-16
			return $op;
		}
	}

	/**
	 * @param int $ms
	 * @param LazyObject $value
	 * 
	 * @return FutureObject
	 */
	static public function delay ($ms, $value) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:189: characters 5-87
		return Future_Impl_::async(function ($cb)  use (&$ms, &$value) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:189: characters 38-86
			Timer::delay(function ()  use (&$cb, &$value) {
				#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:189: characters 66-81
				$cb($value->get());
			}, $ms);
		});
	}

	/**
	 *  Same as `first`, but use `Either` to handle the two different types
	 * 
	 * @param FutureObject $a
	 * @param FutureObject $b
	 * 
	 * @return FutureObject
	 */
	static public function either ($a, $b) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:150: characters 5-71
		return Future_Impl_::first($a->map(Boot::getStaticClosure(Either::class, 'Left')), $b->map(Boot::getStaticClosure(Either::class, 'Right')));
	}

	/**
	 *  Creates a future that contains the first result from `this` or `other`
	 * 
	 * @param FutureObject $this
	 * @param FutureObject $other
	 * 
	 * @return FutureObject
	 */
	static public function first ($this1, $other) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:19: characters 5-32
		$ret = new FutureTrigger();
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:20: characters 5-39
		$l1 = $this1->handle(Boot::getInstanceClosure($ret, 'trigger'));
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:21: characters 5-40
		$l2 = $other->handle(Boot::getInstanceClosure($ret, 'trigger'));
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:22: characters 5-30
		$ret1 = $ret;
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:23: lines 23-24
		if ($l1 !== null) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:24: characters 18-20
			$this2 = $l1;
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:24: characters 7-21
			$ret1->handle(function ($_)  use (&$this2) {
				#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:24: characters 18-20
				$this2->cancel();
			});
		}
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:25: lines 25-26
		if ($l2 !== null) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:26: characters 18-20
			$this3 = $l2;
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:26: characters 7-21
			$ret1->handle(function ($_1)  use (&$this3) {
				#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:26: characters 18-20
				$this3->cancel();
			});
		}
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:27: characters 5-15
		return $ret1;
	}

	/**
	 *  Creates a new future by applying a transform function to the result.
	 *  Different from `map`, the transform function of `flatMap` returns a `Future`
	 * 
	 * @param FutureObject $this
	 * @param \Closure $next
	 * @param bool $gather
	 * 
	 * @return FutureObject
	 */
	static public function flatMap ($this1, $next, $gather = true) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:45: lines 45-50
		if ($gather === null) {
			$gather = true;
		}
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:46: characters 5-34
		$ret = $this1->flatMap($next);
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:48: lines 48-49
		if ($gather) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:48: characters 19-31
			return $ret->gather();
		} else {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:49: characters 12-15
			return $ret;
		}
	}

	/**
	 *  Flattens `Future<Future<A>>` into `Future<A>`
	 * 
	 * @param FutureObject $f
	 * 
	 * @return FutureObject
	 */
	static public function flatten ($f) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:71: characters 5-31
		return new NestedFuture($f);
	}

	/**
	 * @param \Array_hx $futures
	 * 
	 * @return FutureObject
	 */
	static public function fromMany ($futures) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:112: characters 5-27
		return Future_Impl_::ofMany($futures);
	}

	/**
	 * @param LazyObject $l
	 * 
	 * @return FutureObject
	 */
	static public function lazy ($l) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:116: characters 5-29
		return new SyncFuture($l);
	}

	/**
	 *  Creates a new future by applying a transform function to the result.
	 *  Different from `flatMap`, the transform function of `map` returns a sync value
	 * 
	 * @param FutureObject $this
	 * @param \Closure $f
	 * @param bool $gather
	 * 
	 * @return FutureObject
	 */
	static public function map ($this1, $f, $gather = true) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:34: lines 34-39
		if ($gather === null) {
			$gather = true;
		}
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:35: characters 5-27
		$ret = $this1->map($f);
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:37: lines 37-38
		if ($gather) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:37: characters 19-31
			return $ret->gather();
		} else {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:38: characters 12-15
			return $ret;
		}
	}

	/**
	 *  Merges two futures into one by applying the merger function on the two future values
	 * 
	 * @param FutureObject $this
	 * @param FutureObject $other
	 * @param \Closure $merger
	 * @param bool $gather
	 * 
	 * @return FutureObject
	 */
	static public function merge ($this1, $other, $merger, $gather = true) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:63: lines 63-65
		if ($gather === null) {
			$gather = true;
		}
		$ret = $this1->flatMap(function ($t)  use (&$other, &$merger) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:64: characters 14-66
			return $other->map(function ($a)  use (&$t, &$merger) {
				#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:64: characters 39-58
				return $merger($t, $a);
			});
		});
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:63: lines 63-65
		if ($gather) {
			return $ret->gather();
		} else {
			return $ret;
		}
	}

	/**
	 *  Like `map` and `flatMap` but with a polymorphic transformer and return a `Promise`
	 *  @see `Next`
	 * 
	 * @param FutureObject $this
	 * @param \Closure $n
	 * 
	 * @return FutureObject
	 */
	static public function next ($this1, $n) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:57: characters 5-50
		return $this1->flatMap(function ($v)  use (&$n) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:57: characters 38-49
			return $n($v);
		});
	}

	/**
	 * @param mixed $v
	 * 
	 * @return FutureObject
	 */
	static public function ofAny ($v) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:82: characters 5-26
		return new SyncFuture(new LazyConst($v));
	}

	/**
	 *  Merges multiple futures into Future<Array<A>>
	 * 
	 * @param \Array_hx $futures
	 * @param bool $gather
	 * 
	 * @return FutureObject
	 */
	static public function ofMany ($futures, $gather = true) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:93: lines 93-108
		if ($gather === null) {
			$gather = true;
		}
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:94: characters 5-24
		$ret = new SyncFuture(new LazyConst(new \Array_hx()));
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:95: lines 95-104
		$_g = 0;
		while ($_g < $futures->length) {
			unset($f);
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:95: characters 10-11
			$f = ($futures->arr[$_g++] ?? null);
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:96: lines 96-104
			$ret = $ret->flatMap(function ($results)  use (&$f) {
				#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:98: lines 98-102
				return $f->map(function ($result)  use (&$results) {
					#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:100: characters 15-46
					return $results->concat(\Array_hx::wrap([$result]));
				});
			});
		}

		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:106: lines 106-107
		if ($gather) {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:106: characters 19-31
			return $ret->gather();
		} else {
			#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:107: characters 12-15
			return $ret;
		}
	}

	/**
	 *  Same as `first`
	 * 
	 * @param FutureObject $a
	 * @param FutureObject $b
	 * 
	 * @return FutureObject
	 */
	static public function or ($a, $b) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:144: characters 5-22
		return Future_Impl_::first($a, $b);
	}

	/**
	 *  Creates a sync future.
	 *  Example: `var i = Future.sync(1); // Future<Int>`
	 * 
	 * @param mixed $v
	 * 
	 * @return FutureObject
	 */
	static public function sync ($v) {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:123: characters 5-29
		return new SyncFuture(new LazyConst($v));
	}

	/**
	 *  Creates a new `FutureTrigger`
	 * 
	 * @return FutureTrigger
	 */
	static public function trigger () {
		#/home/grabli66/haxelib/tink_core/git/src/tink/core/Future.hx:186: characters 5-31
		return new FutureTrigger();
	}

	/**
	 * @internal
	 * @access private
	 */
	static public function __hx__init ()
	{
		static $called = false;
		if ($called) return;
		$called = true;


		self::$NULL = new SyncFuture(new LazyConst(null));
		self::$NOISE = new SyncFuture(new LazyConst(Noise::Noise()));
		self::$NEVER = NeverFuture::$inst;
	}
}

Boot::registerClass(Future_Impl_::class, 'tink.core._Future.Future_Impl_');
Future_Impl_::__hx__init();
